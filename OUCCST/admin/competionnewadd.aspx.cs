...
public partial class admin_newsadd1 : System.Web.UI.Page
{
    ...
    {
        Response.Redirect("competionnews.aspx");
    }
            ...
            try
            {
                using (var db = new CstwebEntities())
                {
                    ...
                    if (neid == 0)
                    {
                        db.news.Add(ne);
                        db.SaveChanges();
                        Response.Write("<script language=javascript>alert('发布成功');window.location = 'competionnews.aspx';</script>");
                    }
                    else
                    {
                        db.SaveChanges();
                        Response.Write("<script language=javascript>alert('修改成功');window.location = 'competionnews.aspx';</script>");
                    }
