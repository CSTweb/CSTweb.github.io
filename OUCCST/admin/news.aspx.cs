...
public partial class admin_news1 : System.Web.UI.Page
{
    ...
    {
        if (!Int16.TryParse(Request.QueryString["nc"], out newsclass)) newsclass = 1;
        ...
        if (e.CommandName == "del")
        {
            using (var db = new CstwebEntities())
            {
                try
                {
                    ...
                    db.news.Remove(ne);
                    db.SaveChanges();
                    Response.Write("<script>alert('删除成功');window.location = 'news.aspx?nc="+newsclass.ToString()+"';</script>");
                }
    ...
    protected void BtnPreviousPage_Click(object sender, EventArgs e)
    {
        ...
        Response.Redirect("news.aspx?nc=" + newsclass.ToString() + "&page=" + pageNum.ToString());
        //TxtPageNum.Text = pageNum.ToString();
    }
    ...
    {
        Response.Redirect("news.aspx?nc=" + newsclass.ToString() + "&page=1");
    }

    protected void BtnTrailerPage_Click(object sender, EventArgs e)
    {
        Response.Redirect("news.aspx?nc=" + newsclass.ToString() + "&page=" + pageCount.ToString());
    }
    ...
    {
        Response.Redirect("newsadd.aspx?nc="+newsclass.ToString());
    }
    protected void DdlSeClass_SelectedIndexChanged(object sender, EventArgs e)
    {
        ...
        Response.Redirect("news.aspx?nc=" + newsclass.ToString());
    }
}